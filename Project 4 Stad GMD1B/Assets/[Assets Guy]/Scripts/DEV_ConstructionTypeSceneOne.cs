...
public class DEV_ConstructionTypeSceneOne : MonoBehaviour {
    ...
    void Update () {
		if(cutSceneActivated == true && hasSpawned == false) {
            ...
            controller.forcedFirstPerson = true;
        ...
        if(cType != null) {
            Vector3 transformGoal = Vector3.MoveTowards(cType.transform.position, constructionTypeFinalPos.transform.position, Time.deltaTime * spawnSpeedMultiplier);
            ...
            if (cType.transform.position == constructionTypeFinalPos.transform.position) {
                ...
                controller.forcedFlashLightFalse = false;
                controller.forcedFirstPerson = false;
            }
        }
	}
}
